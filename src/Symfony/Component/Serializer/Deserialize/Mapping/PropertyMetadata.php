...
/**
 * Holds deserialization metadata about a given property.
 *
        ...
        foreach ($metadata->formatters as $formatter) {
            ...
            if ($reflection->getClosureScopeClass()?->hasMethod($reflection->getName())) {
                if (!$reflection->isStatic()) {
                    throw new InvalidArgumentException(sprintf('"%s"\'s property deserialize formatter must be a static method.', $metadata->name));
                }
            } else {
                if ($reflection->isAnonymous()) {
                    throw new InvalidArgumentException(sprintf('"%s"\'s property deserialize formatter must not be anonymous.', $metadata->name));
                }
            }
        }
    }
