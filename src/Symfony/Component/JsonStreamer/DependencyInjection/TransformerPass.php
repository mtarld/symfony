...
/**
 * Collects and merge services tagged with "json_streamer.value_transformer"
 * or "json_streamer.value_object_transformer" and sets them as arguments for the
 * services that needs transformers.
 *
...
class TransformerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($container->findTaggedServiceIds('json_streamer.value_transformer', true) as $id => $_) {
            ...
            if (!is_a($class ?? '', ValueTransformerInterface::class, true)) {
                throw new InvalidArgumentException(\sprintf('The service "%s" tagged "json_streamer.value_transformer" must implement "%s".', $id, ValueTransformerInterface::class));
            }
        ...
        foreach ($container->findTaggedServiceIds('json_streamer.value_object_transformer', true) as $id => $_) {
            $class = $container->getParameterBag()->resolveValue($container->getDefinition($id)->getClass());
            if (!is_a($class ?? '', ValueObjectTransformerInterface::class, true)) {
        ...
        // remove deprecated value transformers
        unset(
            $map['json_streamer.value_transformer.string_to_date_time'],
            $map['json_streamer.value_transformer.date_time_to_string'],
        );
        ...
        $container->getDefinition('.json_streamer.cache_warmer.streamer')
            ->replaceArgument(1, $transformersArgument);
    }
}
