...
class AttributePropertyMetadataLoaderTest extends TestCase
{
    ...
    {
        $loader = new AttributePropertyMetadataLoader(new PropertyMetadataLoader(TypeResolver::create()), new ServiceContainer([
            DoubleIntAndCastToStringValueTransformer::class => new DateTimeValueObjectTransformer(),
            BooleanToStringValueTransformer::class => new BooleanToStringValueTransformer(),
        ]), TypeResolver::create());

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage(\sprintf('"%s" is a "%s" and must not be specified as a value transformer.', DoubleIntAndCastToStringValueTransformer::class, ValueObjectTransformerInterface::class));
