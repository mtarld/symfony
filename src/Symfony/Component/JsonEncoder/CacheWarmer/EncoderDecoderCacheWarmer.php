...
final class EncoderDecoderCacheWarmer implements CacheWarmerInterface
{
    ...
    public function __construct(
        ...
        string $encodersDir,
        string $decodersDir,
        private array $encoderConfig,
        private array $decoderConfig,
        private LoggerInterface $logger = new NullLogger(),
    ) {
        $this->encoderGenerator = new EncoderGenerator(new EncodeDataModelBuilder($encodePropertyMetadataLoader), $encodersDir);
        $this->decoderGenerator = new DecoderGenerator(new DecodeDataModelBuilder($decodePropertyMetadataLoader), $decodersDir);
    }
    ...
    private function warmUpEncoders(Type $type): void
    {
        foreach (EncodeAs::cases() as $encodeAs) {
            try {
                $this->encoderGenerator->generate($type, $encodeAs, $this->encoderConfig);
    ...
    private function warmUpDecoders(Type $type): void
    {
        foreach (DecodeFrom::cases() as $decodeFrom) {
            try {
                $this->decoderGenerator->generate($type, $decodeFrom, $this->decoderConfig);
            } catch (ExceptionInterface $e) {
