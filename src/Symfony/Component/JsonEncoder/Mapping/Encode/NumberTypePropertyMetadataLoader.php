...
/**
 * Transforms number object to string for properties with number object type.
 *
...
final class NumberTypePropertyMetadataLoader implements PropertyMetadataLoaderInterface
{
    ...
    public function load(string $className, array $options = [], array $context = []): array
    {
        ...
        foreach ($result as &$metadata) {
            ...
            if ($type->isIdentifiedBy(Number::class, \GMP::class)) {
                $metadata = $metadata
                    ->withType(NumberToStringValueTransformer::getJsonValueType())
                    ->withAdditionalToJsonValueTransformer('json_encoder.value_transformer.number_to_string');
            }
        }
