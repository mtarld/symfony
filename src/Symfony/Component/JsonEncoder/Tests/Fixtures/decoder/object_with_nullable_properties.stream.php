...
return static function (mixed $stream, \Psr\Container\ContainerInterface $denormalizers, \Symfony\Component\JsonEncoder\Decode\LazyInstantiator $instantiator, array $config): mixed {
    $providers['Symfony\Component\JsonEncoder\Tests\Fixtures\Model\DummyWithNullableProperties'] = static function ($stream, $offset, $length) use ($config, $denormalizers, $instantiator, &$providers) {
        ...
        foreach ($data as $k => $v) {
            match ($k) {
                ...
                'enum' => $properties['enum'] = static function () use ($stream, $v, $config, $denormalizers, $instantiator, &$providers) {
                    return $providers['Symfony\Component\JsonEncoder\Tests\Fixtures\Enum\DummyBackedEnum|null']($stream, $v[0], $v[1]);
                },
                default => null,
            };
        }
    ...
    $providers['Symfony\Component\JsonEncoder\Tests\Fixtures\Enum\DummyBackedEnum|null'] = static function ($stream, $offset, $length) use ($config, $denormalizers, $instantiator, &$providers) {
