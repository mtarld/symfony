...
return static function (mixed $stream, array $config, \Symfony\Component\JsonEncoder\Instantiator\LazyInstantiatorInterface $instantiator, ?\Psr\Container\ContainerInterface $services) : mixed {
    ...
    $providers['Symfony\\Component\\Json\\Tests\\Fixtures\\Model\\DummyWithOtherDummies'] = static function ($stream, $offset, $length) use($config, $instantiator, $services, &$providers, $flags) {
        ...
        return $instantiator->instantiate(\Symfony\Component\Json\Tests\Fixtures\Model\DummyWithOtherDummies::class, $properties);
    };
    $providers['Symfony\\Component\\Json\\Tests\\Fixtures\\Model\\DummyWithNameAttributes'] = static function ($stream, $offset, $length) use($config, $instantiator, $services, &$providers, $flags) {
        $boundaries = \Symfony\Component\Json\Template\Decode\Splitter::splitDict($stream, $offset, $length);
        $properties = [];
        foreach ($boundaries as $k => $boundary) {
            match ($k) {
        ...
        return $instantiator->instantiate(\Symfony\Component\Json\Tests\Fixtures\Model\DummyWithNameAttributes::class, $properties);
    };
