...
/**
 * Tags classes specified in the `encoder.encodable_paths` parameter globs as encodable.
 *
...
final readonly class EncodablePass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasParameter('encoder.encodable_paths')) {
            return;
        }

        foreach ($this->getEncodableClassNames($container->getParameter('encoder.encodable_paths')) as $className) {
            $container->register($className, $className)
                ->setAbstract(true)
                ->addTag('container.excluded')
                ->addTag('encoder.encodable');
        }
    }
