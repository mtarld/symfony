...
namespace Symfony\Component\Marshaller\Internal\Denormalizer;
...
final class Denormalizer
{
     ...
     */
    public function denormalize(mixed $data, Type|UnionType $type, array $context): mixed
    {
        ...
        if ($type->isDict()) {
            $result = $this->denormalizeDict($data, $type->collectionValueType(), $context);

            return $type->isIterable() ? $result : iterator_to_array($result);
        }

        if ($type->isList()) {
            $result = $this->denormalizeList($data, $type->collectionValueType(), $context);
                ...
                try {
                    if (null !== $hook = $this->hookExtractor->extractFromKey($reflection->getName(), $key, $context)) {
                        $hook(
                            ...
                            $object,
                            $key,
                            fn (string $type, array $context): mixed => $this->denormalize($value, Type::createFromString($type), $context),
                            $context,
                        );
     ...
     */
    private function denormalizeDict(array $data, Type|UnionType $type, array $context): \Iterator
    {
        foreach ($data as $key => $value) {
            yield $key => $this->denormalize($value, $type, $context);
        }
    }
     ...
     */
    private function denormalizeList(array $data, Type|UnionType $type, array $context): \Iterator
    {
        foreach ($data as $value) {
            yield $this->denormalize($value, $type, $context);
        }
    }
     ...
     */
    // TODO can be overriden in context?
